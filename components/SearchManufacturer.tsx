...
const SearchManufacturer = ({ manufacturer, setManufacturer }: SearchManufacturerProps) => {
    ...
    const filteredManufaturers = Query === '' ? manufacturers : manufacturers.filter((item) => (
                    ...
                    >
                        <Combobox.Options>
                            {filteredManufaturers.map((item) => (
                                <Combobox.Option key={item} className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-primary-blue' : 'text-gray-900'}`}
                                    value={item}
                                >
                                    {({ selected, active }) => (
                                        <>
